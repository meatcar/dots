...
in {
  ...
  in
    lib.mkOption {
      type = with lib.types; attrsOf (submodule {options = account;});
      description = "An attrset of <account nickname> => <account props>";
    };
  config = let
    accounts =
      builtins.mapAttrs (name: account: {
        ...
        vdirsyncer = {
          enable = true;
          clientIdCommand = ["cat" "${config.age.secrets.gcalClientId.path}"];
          clientSecretCommand = ["cat" "${config.age.secrets.gcalClientSecret.path}"];
