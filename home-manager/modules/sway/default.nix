{
  ...
  gebaar-libinput =
    pkgs.gebaar-libinput.overrideAttrs
    (attrs: rec {
      version = "2019-11-29";
...
in {
    ...
    config = {
      ...
      in
        lib.mkDefault {
          ...
          "${mod}+d" = "kill";
          "${mod}+Shift+d" = "exec fzf-wrap-exec \"ps -f -U $USER | sed 1d | fzf ${fzfOpts} | awk '{print \\$2'}' | xargs kill\"";
          "${mod}+p" = "exec ${menu}";
          "${mod}+Shift+p" = "exec fzf-wrap-exec \"dmenu_path | fzf ${fzfOpts} --history=\$HOME/.cache/sway/launch-hist\"";
  ...
  xdg.configFile."gebaar/gebaard.toml".text = let
